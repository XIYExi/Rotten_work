...
export default function FooterComponent() {
  ...
  const [data, setData] = useState<{ [key:string]: FooterItemProps[] }>({});
  // @ts-ignore
  useEffect(async () => {
    const {default:Graph} = await import('@/assets/data/FooterColumnData');
      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-12 gap-x-[30px] mb-[-30px]">
          ...
          <div className="col-span-12 sm:col-span-6 lg:col-span-4 mb-[30px]">
            <a href='#' className='block mb-[25px]'>
              <img src={logo} width='99' height='46' loading='lazy' alt='footer log'/>
            </a>

            <p className='mb-[5px] xl:mb-[40px] max-w-[270px]'>
              Properties are most budget friendly so you have are find opportunity is main responsibility to clients
            </p>

            <p className='text-sm hidden md:block'>
              &copy;<a href="#" className="text-secondary">CodeCarnival.</a>
          ...
          {/*第二 | 三栏*/}
          {
            Object.keys(data).map((item:string, index:number) =>(
              <div key={index} className='col-span-12 sm:col-span-6 lg:col-span-3 mb-[30px]'>
                <h3 className='font-recoleta font-normal text-[22px] leading-[1.222] text-primary mb-[20px] lg:mb-[30x]'>
                  {
                    /*@ts-ignore*/
                    data[item].name}<span className='text-secondary'>.</span>
                </h3>

                <ul className='text-[16px] leading-none mb-[20px]'>
                  {
                    /*@ts-ignore*/
                    data[item].columnItem.map((_item: ColumnItemProps, index: number) => (
                      <li className='mb-[20px]' key={index}>
                        <a href={_item.href} className='inline-block transition-all hover:text-secondary'>
                          {_item.name}
                        </a>
                      </li>
                    ))
          ...
          <div className='col-span-12 sm:col-span-6 lg:col-span-2 mb-[30px]'>
            <h3 className='font-recoleta font-normal text-[22px] leading-[1.222] text-primary mb-[20px] lg:mb-[30px]'>
            ...
            <ul className='inline-flex items-center justify-center mt-[20px]'>
              {
                SvgIcon.map((item:ReactNode, index:number) => (
                  <li key={index} className="first:ml-0 ml-[15px]">
                    <a href="#"
                      className="w-[26px] h-[26px] transition-all rounded-full bg-primary flex items-center justify-center
                       hover:drop-shadow-modify text-white hover:bg-secondary">{item}</a>
                  </li>
                ))
              }
            </ul>
...
const SvgIcon:ReactNode[] = [
  <svg
    width="7" height="12" viewBox="0 0 7 12" fill="currentColor"
